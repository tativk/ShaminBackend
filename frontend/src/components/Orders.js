...
import{FiPlus,FiSearch,FiFilter,FiChevronDown,FiChevronLeft,FiChevronRight,FiCalendar,FiUsers,FiDatabase,FiCheckCircle,FiTruck,FiClock,FiXCircle,FiEye,FiMoreHorizontal,FiCopy,FiPackage,FiRefreshCw,FiShoppingBag,FiX,FiMapPin,FiPhone,FiCreditCard,FiUser,FiHash,FiCheck,FiEdit3}from"react-icons/fi";
import"./Orders.css";
...
function Orders(){
const[rows,setRows]=useState(INIT),[q,setQ]=useState(""),[fst,setFst]=useState("all"),[fcu,setFcu]=useState("all"),[fpy,setFpy]=useState("all"),[fdt,setFdt]=useState("all"),[srt,setSrt]=useState("newest"),[pg,setPg]=useState(1),[rpp,setRpp]=useState(6),[om,setOm]=useState(null),[omPos,setOmPos]=useState(null),[cp,setCp]=useState(null),[ct,setCt]=useState(false),[mf,setMf]=useState(false),[pmin,setPmin]=useState(""),[pmax,setPmax]=useState(""),[sel,setSel]=useState(null),[dop,setDop]=useState(false),[ed,setEd]=useState(false),[ec,setEc]=useState(""),[ep,setEp]=useState(""),[ea,setEa]=useState(""),[ect,setEct]=useState("مشتری عادی"),[epy,setEpy]=useState("پرداخت آنلاین"),[es,setEs]=useState("pending"),[epr,setEpr]=useState([]),[nop,setNop]=useState(false),[nc,setNc]=useState(""),[np,setNp]=useState(""),[nct,setNct]=useState("مشتری عادی"),[nad,setNad]=useState(""),[npy,setNpy]=useState("پرداخت آنلاین"),[npr,setNpr]=useState(""),[nq,setNq]=useState("1"),[na,setNa]=useState("");
const stats=useMemo(()=>({total:rows.length,completed:rows.filter(o=>o.status==="completed").length,shipping:rows.filter(o=>o.status==="shipping").length,pending:rows.filter(o=>o.status==="pending").length,cancelled:rows.filter(o=>o.status==="cancelled").length}),[rows]);
...
const hsc=nx=>{if(nx==="cancelled"&&sel.status!=="cancelled"&&!window.confirm("آیا مطمئن هستید که می‌خواهید این سفارش لغو شود؟"))return;setRows(p=>p.map(o=>o.id===sel.id?{...o,status:nx,statusLabel:SC[nx].label}:o));setSel(p=>p?{...p,status:nx,statusLabel:SC[nx].label}:p)};
const hno=e=>{e.preventDefault();if(!nc.trim()||!npr.trim()||!Number(na))return;const n=Math.max(...rows.map(o=>Number(o.id.replace(/\D/g,""))))+1,qty=Math.max(1,Number(nq)||1),price=Number(na);const no={id:"SH-"+n,customer:nc.trim(),customerType:nct,phone:np.trim()||"ثبت نشده",address:nad.trim()||"آدرس ثبت نشده",payment:npy,products:[{name:npr.trim(),type:"محصول",quantity:qty,price}],productCount:qty,amount:price*qty,status:"pending",statusLabel:"در انتظار",time:"همین الان",date:"امروز"};setRows(p=>[no,...p]);setNc("");setNp("");setNpr("");setNa("");setNct("مشتری عادی");setNad("");setNpy("پرداخت آنلاین");setNq("1");setNop(false);setPg(1)};
const tglOm=(oId,e)=>{
...
const step=sel?{pending:1,shipping:3,completed:4,cancelled:0}[sel.status]:0;
return(
...
<div className="so-fg"><label>وضعیت سفارش</label><div className="so-os">{[{value:"all",label:"همه",count:stats.total},{value:"completed",label:"تکمیل شده",count:stats.completed},{value:"shipping",label:"در حال ارسال",count:stats.shipping},{value:"pending",label:"در انتظار",count:stats.pending},{value:"cancelled",label:"لغو شده",count:stats.cancelled}].map(i=>(<button key={i.value} type="button" className={"so-op"+(fst===i.value?" so-opa":"")} onClick={()=>chf(setFst,i.value)}><span className="so-cb">{fst===i.value&&<FiCheck/>}</span><span>{i.label}</span><small>{i.count.toLocaleString("fa-IR")}</small></button>))}</div></div>
<div className="so-fd"/>
...
{cur.length>0?cur.map(o=>{const I=SC[o.status].icon;return(
...
<div className="so-db">
<div className="so-dstt">{(()=>{const I=SC[sel.status].icon;return<div className={"so-dsti so-dsti-"+sel.status}><I/></div>})()}<div><span>وضعیت سفارش</span><strong>{sel.statusLabel}</strong></div><div className="so-dsc"><label htmlFor="so-sts">تغییر وضعیت</label><select id="so-sts" value={sel.status} onChange={e=>hsc(e.target.value)}><option value="pending">در انتظار</option><option value="shipping">در حال ارسال</option><option value="completed">تکمیل شده</option><option value="cancelled">لغو شده</option></select><FiChevronDown/></div></div>

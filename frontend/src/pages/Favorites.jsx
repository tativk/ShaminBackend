...
const Favorites = () => {
  ...
  const addToCart = async (product) => { try { await apiRequest("/cart/items/", { method: "POST", body: JSON.stringify({ product: product.id, quantity: 1 }) }); setNotice("محصول به سبد خرید اضافه شد."); } catch (error) { setNotice(error.message || "افزودن به سبد خرید انجام نشد."); } };
